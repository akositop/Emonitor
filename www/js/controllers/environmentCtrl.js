app.controller('environmentCtrl', function($scope, $cordovaFile, $cordovaFileTransfer, $cordovaDevice,$timeout,$ionicLoading,$ionicHistory ,$ionicPlatform, $cordovaCamera, $ionicActionSheet, $ionicPopup, ionicMaterialInk, $stateParams, $rootScope, $state, Service, envi, Data) {
      ...
      $scope.saveEnvi = function(envi) {
        ...
        // Destination URL
              ...
              $cordovaFileTransfer.upload(url, targetPath, options).then(function (result) {
                  ...
                  envi.envi_img = filename;
                  Service.insertEnvi(envi).then(function (yow) {
                      /////////for time out nga overlay nga loading///////////                
                      $timeout(function(){
                         $ionicLoading.hide();
                       },200);
                     /////////////////////////////////////////////////////// 
                      $scope.envi.envi_findings = " ";
                      $state.go('menu.top5');
          ...
          Service.updateEnvi(enviID, envi).then(function(yow){
          alert(yow.status);
          $state.go('menu.enviReports');
        });
        }
        // $state.go('/');
      };
...
//Image Controll//////////////////////////////////////////////////
  ...
  //Populate ang select/combobox
    Service.getlocation().then(function(data){
    $scope.locations = data.data;
    });

    Service.getoperation().then(function(data){
    $scope.operations = data.data;
    });
    
    Service.getCriteria().then(function(data){
